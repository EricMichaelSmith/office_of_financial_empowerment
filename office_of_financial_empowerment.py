...
def main():
    ...
    # Join databases
    mysql_s = """
        CREATE TABLE clients_expanded
        SELECT *
        FROM clients LEFT JOIN {counselor_table_name}
        ON clients.ssn = {counselor_table_name}.form_ssn;
        """
    cur.execute(mysql_s.format(counselor_table_name=counselor_table_name))
    
    # Output results
    cur.execute('SELECT * FROM clients_expanded'.format(table_name=counselor_table_name))
    rows = cur.fetchall()
    for row in rows:
        print(row)
...
def counselor_form_input():
    ...
    mysql_s = """
        ...
        PRIMARY KEY (form_ssn)
        );
        """
    cur.execute(mysql_s.format(table_name=table_name))
    mysql_s = """
        INSERT INTO {table_name} (form_ssn, next_meeting_date, office_address, grow_personal_income, bank_to_use, bank_savings_per_month, deposit_goal, current_bank_name)
